...
@app.post("/quake")
def add_quake(magnitude: float, location: str):
    severity = "Minor"

    if magnitude >= 5.0 and magnitude < 7.0:
        severity = "Moderate"
    elif magnitude >= 7.0:
        severity = "Severe"
...
Add Earthquake Record
POST /quake

Example URL

http://127.0.0.1:8000/quake?magnitude=6.2&location=Kathmandu
...

{
    "status": "saved",
    "data": {
        "timestamp": "2026-05-28 16:15:00",
        "magnitude": 6.2,
        "location": "Kathmandu",
        "severity": "Moderate"
    }
}
Retrieve Earthquake Records
GET /quake

URL

http://127.0.0.1:8000/quake
...
The database stores all earthquake records in JSON format.
...
API Development	Successful	REST endpoints implemented
Server Execution	Successful	Running on localhost:8000
POST Request	Successful	Earthquake data saved
GET Request	Successful	Records retrieved successfully
Severity Classification	Successful	Minor, Moderate and Severe categories implemented
...
All data is permanently stored in quake_db.json.
...
Smart city disaster monitoring systems
...
The API was successfully tested using GET and POST requests, confirming correct functionality. 
This project illustrates how lightweight REST APIs can be developed for real-time monitoring 
applications and can be further extended by integrating cloud services, IoT seismic sensors, GIS mapping, and real-time alert systems.
